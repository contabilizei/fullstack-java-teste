...
//		}];
		...
		/**
		 * Pesquisa um imposto por nome na base de dados
		 */
		vm.pesquisar = function() {
			ImpostoService.pesquisarPorNome(vm.search).then(
					function sucess(result) {
		 ...
		 */
		 vm.salvar = function(form) {
		        vm.submitted = true;
		        if (form.$valid) {
		            	ImpostoService.salvar(vm.entity)
		    			.then(function sucess(result) {
		                    $state.go('imposto',{msg: 'Imposto incluída com sucesso',type: 'success'});
		    			},function error(response) {
		    				vm.mensagemErro(response);
		    			});
		            }
